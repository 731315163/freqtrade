...
from freqtrade.data.history import history_utils
from . import hp,cmd
from utils import pathutil, timeutil, TimeFrameStr,dfutil,DatetimeType,TimedeltaType

logger = logging.getLogger("freqtrade")
timetypes: TypeAlias = timeutil.DatetimeType
timedeltaUnion: TypeAlias = timeutil.TimedeltaType
...
class TradeBot:
    ...
    def __init__(
        self,
        user_data_path: Path,
        timeframe: timedeltaUnion | None = None,
    ...
    def add_basecommands(
        ...
        timeframe: timedeltaUnion | None = None,
        timeframedetail: timedeltaUnion | None = None,
        starttime: timetypes | None = None,
        endtime: timetypes | None = None,
        ...
        if starttime :
            commands += [cmd.timerange, TradeBot.timestr(start=starttime, end=endtime)]
        if strategyorname :
    ...
    def lookahead_analysis(
        ...
        timeframe: timedeltaUnion | None = None,
        timeframedetail: timedeltaUnion | None = timedelta(minutes=1),
    ):
        nowdatastr = timeutil.now().strftime("%Y%m%d_%H%M")
        commandslist = [
            "lookahead-analysis",
            cmd.strategy,
            self.strategy_name,
    ...
    def hyperparameter_optimize(
        ...
        timerange = TradeBot.timestr(start, end)
    ...
    @staticmethod
    def timestr(start: timetypes | None = None, end: timetypes | None = None):
        if start is not None:
            timestr = timeutil.to_nonetz(start).strftime("%Y%m%d") + "-"
        else:
            timestr = "-"
        if end is not None:
            timestr += timeutil.to_nonetz(end).strftime("%Y%m%d")
        return timestr

    @overload
    def download(
        self,
        start_date: timetypes | None = None,
        end_date: timetypes | None = None,
    ...
    def get_arguments(self, sysargv: MutableSequence):
        sysargv = TradeBot._elements_to_str(sysargv)

        return sysargv

    def _run(self, sysargv: MutableSequence[str]):
        """
